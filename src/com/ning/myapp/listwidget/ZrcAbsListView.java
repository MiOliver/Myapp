package com.ning.myapp.listwidget;

import java.util.ArrayList;
import java.util.List;

import com.ning.myapp.R;
import com.ning.myapp.listwidget.ZrcListView.OnItemLongClickListener;
import com.ning.myapp.listwidget.ZrcListView.OnScrollListener;
import com.ning.myapp.listwidget.ZrcListView.OnScrollStateListener;
import com.ning.myapp.listwidget.ZrcListView.OnStartListener;
import com.ning.myapp.utils.APIUtil;
...
public abstract class ZrcAbsListView extends ZrcAdapterView<ListAdapter>
	...
	private Runnable mPendingCheckForTap, mLongPressRunnable;
	...
	private OnStartListener onRefreshStart;
	...
	public ZrcAbsListView(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.ZrcAbsListView, defStyle, 0);
		Drawable d = a
				.getDrawable(R.styleable.ZrcAbsListView_android_listSelector);
		if (d != null) {
			setSelector(d);
		}
		mDrawSelectorOnTop = a.getBoolean(
				R.styleable.ZrcAbsListView_android_drawSelectorOnTop, false);
		boolean scrollingCacheEnabled = a.getBoolean(
				R.styleable.ZrcAbsListView_android_scrollingCache, true);
		setScrollingCacheEnabled(scrollingCacheEnabled);
		int transcriptMode = a.getInt(
				R.styleable.ZrcAbsListView_android_transcriptMode,
				TRANSCRIPT_MODE_DISABLED);
		setTranscriptMode(transcriptMode);
		int color = a.getColor(
				R.styleable.ZrcAbsListView_android_cacheColorHint, 0);
		setCacheColorHint(color);
		boolean smoothScrollbar = a.getBoolean(
				R.styleable.ZrcAbsListView_android_smoothScrollbar, true);
	...
	protected int computeVerticalScrollOffset() {
		...
		if (firstPosition >= 0 && childCount > 0) {
			if (mSmoothScrollbarEnabled) {
				...
				if (height > 0) {
					return Math.max((firstPosition * range - top * range
							/ height)
							/ count + mFirstTopOffset * 10 + mScrollY * 10,
							mFirstTopOffset * 10);
				}
			} else {
				int index;
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (zrcHeader != null) {
			...
			if (needShowHeader) {
				boolean more = zrcHeader.draw(canvas, listPadding.left, viewTop
						+ mFirstTopOffset, getWidth() - listPadding.right,
						firstTop);
		...
		if (childCount > 0 && (isRefreshing || isLoadingMore) && footer != null) {
			...
			if (firstPosition + childCount == mItemCount) {
				...
				if (lastBottom < viewBottom) {
					boolean more = footer.draw(canvas, listPadding.left,
							lastBottom, getWidth() - listPadding.right,
	...
	private boolean startScrollIfNeeded(int x, int y) {
		...
		if (distanceX > mTouchSlop || distanceY > mTouchSlop) {
			if (distanceY > distanceX * 2) {
				...
				setPressed(false);
				if (getChildCount() > 0) {
					final View motionView = getChildAt(mMotionPosition
							- mFirstPosition);
					if (motionView != null) {
	...
	private void onTouchUp(MotionEvent ev) {
		...
		case TOUCH_MODE_DONE_WAITING:
			...
			final View child = getChildCount() == 0 ? null
					: getChildAt(motionPosition - mFirstPosition);
			if (child != null) {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN: {
			...
			if (touchMode != TOUCH_MODE_FLING
					&& touchMode != TOUCH_MODE_RESCROLL && motionPosition >= 0) {
				mMotionPosition = motionPosition;
				mTouchMode = TOUCH_MODE_DOWN;
				clearScrollingCache();
			}
	...
	public boolean canScrollList(int direction) {
		...
		if (direction > 0) {
			...
			return firstPosition > 0
					|| firstTop < listPadding.top + mFirstTopOffset;
		}
	}
	...
	private void setRefreshDone(String msg, int state) {
		...
		if (zrcHeader != null && zrcHeader.getState() == Headable.STATE_LOADING) {
			...
			final boolean unShownState = mFirstPosition > 0
					|| firstTop < mListPadding.top + mFirstTopOffset
							+ zrcHeader.getHeight();
	...
	private class FlingRunnable implements Runnable {
		...
		boolean scrollToAdjustViewsUpOrDown() {
			...
			final int firstTop = childCount == 0 ? mFirstTop : getChildAt(0)
					.getTop();
			int lastBottom = childCount == 0 ? firstTop : getChildAt(
					childCount - 1).getBottom();
			if (firstPosition + childCount >= mItemCount - 1) {
				if (!isRefreshing && !isLoadingMore && isLoadMoreOn
						&& onLoadMoreStart != null) {
					isLoadingMore = true;
			...
			final Rect listPadding = mListPadding;
			final Headable zrcHeader = mZrcHeader;
			final boolean isOnLoading = zrcHeader != null
					&& zrcHeader.getState() == Headable.STATE_LOADING;
			final int topOffset = firstTop
					- (listPadding.top + mFirstTopOffset + (showHeader ? zrcHeader
							.getHeight() : 0));
			...
			final boolean isTooShort = childCount == mItemCount
					&& lastBottom - firstTop < getHeight();
			final int bottomOffset = isTooShort ? firstTop - listPadding.top
					- mFirstTopOffset : lastBottom - getHeight()
					+ listPadding.bottom + mLastBottomOffset;
			final boolean cannotScrollUp = firstPosition + childCount == mItemCount
					&& bottomOffset < 0;
			if (cannotScrollDown) {
				int duration = mFirstTopOffset + listPadding.top - firstTop;
				if ((zrcHeader != null && onRefreshStart != null && zrcHeader
						.getState() == Headable.STATE_RELEASE)) {
					if (!isOnLoading) {
						isRefreshing = true;
						showHeader = true;
				...
				startScroll(-duration,
						(int) Math.abs(duration / mDensity) + 50, true);
				mTouchMode = TOUCH_MODE_RESCROLL;
	...
	class AdapterDataSetObserver extends
			ZrcAdapterView<ListAdapter>.AdapterDataSetObserver {
		@Override
